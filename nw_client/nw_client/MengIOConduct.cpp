...
bool MengIOConduct::recvive(SOCKET tcp_socket, MengTcpClient& tcp_client)
{
    ...
    while (1) {
        if (!tcp_client.recv(buffer, sizeof(buffer), tcp_socket)) {
...
bool MengIOConduct::sendBinaryFile(SOCKET tcp_socket, string path, MengTcpClient& tcp_client)
{
    memset(&buffer, 0, sizeof(buffer));
    ...
    if ((filesystem::status(path).permissions() & filesystem::perms::owner_read) != filesystem::perms::none) {
        while (reader.read(buffer, sizeof(buffer))) {
            if (!tcp_client.send(buffer, tcp_socket)) {
